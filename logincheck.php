...
// check if they exist in the database
  if(isset($_POST["student_id"]) && isset($_POST["password"])){
    if ($stmt = $mysqli->prepare("select student_id,password         from person where student_id = ? and password = ?")){
	  $input['student_id'] = $_POST["student_id"];
	  $input['password'] = md5($_POST["password"]);
      $stmt->bind_param("ss", $input['student_id'], $input['password']);
      $stmt->execute();
      $stmt->bind_result($p_id, $password);
	    //if there is a match set session variables and send user to homepage
        if ($stmt->fetch()) {
		  $_SESSION["student_id"] = $p_id;
